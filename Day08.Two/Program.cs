...
{
    var antinodes = CalculateAntinodes(grid, kv.Value).Distinct();
...
static IEnumerable<Vector2i> CalculateAntinodes(Grid grid, List<Vector2i> antennae)
{
    foreach (var antenna in antennae)
    {
        foreach (var otherAntenna in antennae)
        {
            ...
            for (var i = 0; ; i++)
            {
                var antinodeA = antenna + (diff * i);
                var antinodeB = otherAntenna - (diff * i);

                if (!grid.IsInBounds(antinodeA) && !grid.IsInBounds(antinodeB))
                    break;
